...
use moxie_dom::{
    elements::html::div,
    interfaces::node::{Child, NodeBuilder},
    prelude::*,
};
...
pub async fn render_test<Root>(render_child: impl FnMut() -> Root + 'static, expected: &str)
where
    Root: Child + 'static,
{
    ...
    assert_eq!(test_root.first_child().unwrap().to_string(), expected,);
}
